<template>
	<div class="triage">
		<h3
			v-if="triage.serious.length"
			class="triage__header"
		>Serious observations: {{ triage.serious.length }}</h3>
		<div v-for="item in triage.serious" :key="item.id" class="triage__item">
			<h5
				:class="item.isEmergency ? 'triage__item--danger' : 'triage__item--warning'"
			>{{ item.common_name }}</h5>
		</div>
