	...
	<div class="condition-list">
		<div v-for="condition in conditions" :key="condition.id">
			<div class="condition-list__item">
				...
				<div
					v-if="condition.probability > 0.2"
					class="condition-list--explain"
			...
			<div class="condition-list__progress progress">
				<div
					:style="getCalculatedWidth(condition.probability, 'width')"
					:aria-valuenow="condition.probability * 100"
					aria-valuemin="0"
					aria-valuemax="100"
					class="progress__bar"
				>{{ getCalculatedWidth(condition.probability) }}</div>
			</div>
			<div class="condition-list__explain explain">
				<div
					v-if="condition.supporting_evidence && condition.supporting_evidence.length > 0"
				...
				>
					...
					<div
						v-for="exp in condition.supporting_evidence"
						:key="exp.id"
						class="explain__condition"
					>{{ exp.common_name }}</div>
				</div>
				<div
					v-if="condition.conflicting_evidence && condition.conflicting_evidence.length > 0"
					class="explain__evidences"
				>
					<div class="explain__header">
					...
					<div
						v-for="exp in condition.conflicting_evidence"
						:key="exp.id"
						class="explain__condition"
					>{{ exp.common_name }}</div>
				</div>
				<div
					v-if="condition.unconfirmed_evidence && condition.unconfirmed_evidence.length > 0"
					class="explain__evidences"
				>
					<div class="explain__header">
					...
					<div
						v-for="exp in condition.unconfirmed_evidence"
						:key="exp.id"
						class="explain__condition"
					>{{ exp.common_name }}</div>
				</div>
